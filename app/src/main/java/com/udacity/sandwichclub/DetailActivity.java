...
public class DetailActivity extends AppCompatActivity {
    ...
    private void populateUI(Sandwich sandwich) {
        ...
        origin.setText(ifUnknown(sandwich.getPlaceOfOrigin()));
        description.setText(ifUnknown(sandwich.getDescription()));
        List<String> list = sandwich.getIngredients();
        String text = list.toString().replace("[", "").replace("]", "");
        ingredients.setText(ifUnknown(text));
        list = sandwich.getAlsoKnownAs();
        text = list.toString().replace("[", "").replace("]", "");
        alsoKnownAs.setText(ifUnknown(text));


    }

    public String ifUnknown(String s) {
        if (s.equals("")) {
            return "N/A";
        } else {
            return s;
